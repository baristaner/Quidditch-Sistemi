...
ENT.BroomModel = "models/models/thunder/thunder.mdl" 
...
if CLIENT then
	...
	local paddingx2 = 10
		...
		local y3 = paddingy3 + 100 + 10

	hook.Add("HUDPaint", "broom_drawhud", function()
		local ent = LocalPlayer():GetNWEntity("broom_active")
		if not ent:IsValid() then return end

		 if( ent:GetNWBool("CanThrowQuaffle") == true ) then
            local paddingx2 = 10
            local paddingy3 = 10
		...
		if GetConVar("brooms_drawcircle") and GetConVar("brooms_drawcircle"):GetBool() then
			...
			surface.DrawCircle(ScrW() / 2, ScrH() / n, 10, Color(0, 255, 255))
			--draw.SimpleText("Quaffle Sende", "broom_font1",x3,y3,Color(255, 69, 0),TEXT_ALIGN_CENTER)
		end

if( LocalPlayer():GetNWBool("bt_quaffle") == true ) then
